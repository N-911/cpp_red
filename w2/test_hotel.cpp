...
public:
    void Book(int64_t time, const string& hotel_name,
        ...
        current_time_ = time;
        hotels_[hotel_name].Book({time, client_id, room_count});///L*logQ
    }
    int ComputeClientCount(const string& hotel_name) {
        return hotels_[hotel_name].ComputeClientCount(current_time_);///L*Q*logQ
    }
    int ComputeRoomCount(const string& hotel_name) {///L*Q*logQ
...
//**************************
    ...
    public:
        void Book(const Booking& booking) {///logQ
            ...
            ++client_booking_counts_[booking.client_id];///logQ
        }
        int ComputeClientCount(int64_t current_time) {///QlogQ
            RemoveOldBookings(current_time);
            return client_booking_counts_.size();
        }
        int ComputeRoomCount(int64_t current_time) {///QlogQ
            RemoveOldBookings(current_time);
            return room_count_;
        }
    private:
        ...
        void PopBooking() {///logQ
            const Booking& booking = last_bookings_.front();
            room_count_ -= booking.room_count;
            const auto client_stat_it =
                    client_booking_counts_.find(booking.client_id); ///logQ
            if (--client_stat_it->second == 0) {
                client_booking_counts_.erase(client_stat_it); ///logQ
            }
            last_bookings_.pop();
        }
        void RemoveOldBookings(int64_t current_time) {///QlogQ
            while (!last_bookings_.empty()
                   && last_bookings_.front().time
                      <= current_time - TIME_WINDOW_SIZE) { ///Q
                PopBooking();///logQ
            }
        }
    };
