...
public:
  ...
  AirportCounter() {
      size_t end_2 = static_cast<size_t>(TAirport::Last_);
      for (auto i = 0; i < end_2; ++i) {
          database[i].first = static_cast<TAirport>(i);
      }
  }
  ...
  AirportCounter(TIterator begin, TIterator end) : AirportCounter() {
      for (auto it = begin; it != end; ++it) {
          database[static_cast<size_t>(*it)].first = *it;
          ++(database[static_cast<size_t>(*it)]).second;
      }
  }
  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const {
      return  database[static_cast<size_t>(airport)].second;
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
      database[static_cast<size_t>(airport)].first = airport;
      ++(database[static_cast<size_t>(airport)]).second;
  }
  ...
  void EraseAll(TAirport airport) {
      database[static_cast<size_t>(airport)].second = 0;
  }
