...
/*
 * функция, разбивающая токены на предложения
 * */
...
vector<Sentence<Token>> SplitIntoSentences(vector<Token> tokens) {
    ...
    for (; pos_start != tokens.end(); ) {
        auto pos_next = find_if(pos_start, tokens.end(), [](auto &it)
                                                    { return (it).IsEndSentencePunctuation(); }
                                                    );
        pos_next = find_if(pos_next, tokens.end(),
                                                    [](auto &it) { return !(it).IsEndSentencePunctuation(); }
                                                    );
        Sentence<Token> temp;
        move(pos_start, pos_next, back_inserter(temp));
